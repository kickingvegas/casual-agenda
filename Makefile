...
LISP_DIR=./lisp
MAIN_EL=$(LISP_DIR)/casual-agenda.el
VERSION_EL=$(LISP_DIR)/casual-agenda-version.el
...
bump-casual:
	...
	sed -i 's/(defconst casual-agenda-version "$(VERSION)"/(defconst casual-agenda-version "$(VERSION_BUMP)"/' $(VERSION_EL)
...
new-sprint: sync-development-with-main
	...
	sed -i 's/(defconst casual-agenda-version "$(VERSION)"/(defconst casual-agenda-version "$(VERSION_PRERELEASE)"/' $(MAIN_EL)
